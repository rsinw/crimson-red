...
local function newAutoAttackSkill(name, cdMax, animLock, effectFn)
    return {
        ...
        use = function(w, b, id, skill)
            ...
            local sd = w.side[id]
            if sd and sd.s == 1 then
                local lk = w.locks[id]
                if lk then lk.moveLock = math.max(lk.moveLock, animLock) end
            end
            local ec = w.effects_comp[id]
            if ec then table.insert(ec.pending, effectFn(id, et.id, b)) end
            skill.cd = (w.stats[id] and statGet(w.stats[id].ATK_SPEED)) or skill.cdMax
        end,
    }
end

function M.newKnightAutoAttackSkill(SCALE, GRAVITY)
    return newAutoAttackSkill("KnightAutoAttack", 2.0, 4*6/60, function(srcId, tgtId, battle)
...
function M.newSkeletonAutoAttackSkill(SCALE, GRAVITY)
    return newAutoAttackSkill("SkeletonAutoAttack", 2.0, 8*6/60, function(srcId, tgtId, battle)
...
function M.newNomadAutoHealSkill(SCALE)
    return newAutoAttackSkill("NomadAutoHeal", 2.5, 8*6/60, function(srcId, tgtId, battle)
...
function M.newBrigandAutoAttackSkill(SCALE, GRAVITY)
    return newAutoAttackSkill("BrigandAutoAttack", 2.0, 7*6/60, function(srcId, tgtId, battle)
        return effects_mod.newTimedAction(srcId, tgtId, 5*6/60, function(w, b, eff)
            if w.entities[eff.tgt] then combat.registerHit(w, battle, eff.src, eff.tgt, {"Melee"}, false, SCALE, GRAVITY) end
        end)
    end)
end

function M.newChampionAutoAttackSkill(SCALE, GRAVITY)
    return newAutoAttackSkill("ChampionAutoAttack", 2.0, 4*6/60, function(srcId, tgtId, battle)
        return effects_mod.newTimedAction(srcId, tgtId, 3*6/60, function(w, b, eff)
            if w.entities[eff.tgt] then combat.registerHit(w, battle, eff.src, eff.tgt, {"Melee"}, false, SCALE, GRAVITY) end
        end)
    end)
end

function M.newDuelistAutoAttackSkill(SCALE, GRAVITY)
    return newAutoAttackSkill("DuelistAutoAttack", 2.0, 6*6/60, function(srcId, tgtId, battle)
        return effects_mod.newTimedAction(srcId, tgtId, 5*6/60, function(w, b, eff)
            if w.entities[eff.tgt] then combat.registerHit(w, battle, eff.src, eff.tgt, {"Melee"}, false, SCALE, GRAVITY) end
        end)
    end)
...
function M.newNomadSparkOfInspirationSkill()
    return {
        ...
        cd=0, cdMax=10.0, resourceCost=0, iconScale=1.0, iconScaleTarget=1.0,
        use = function(w, b, id, skill)
            local et = w.entityTarget[id]; if not et then return end
            local ac = w.anim[id]
            if ac and ac.list[4] then ac.list[4].active=true; ac.list[4].timer=0 end
            ...
            if ec then
                table.insert(ec.pending, effects_mod.newTimedAction(id, et.id, 24/60, function(w2, b2, eff)
                    if w2.entities[eff.tgt] then
                        ...
                        if b2.resource then b2.resource = math.min(b2.resourceMax or 50, b2.resource + 20) end
                    end
                end, {skillSlot=3}))
            end
        end,
...
function M.autoAttackSystem(world)
    for id in pairs(world.entities) do
        ...
        if not world.isRanged[id] and mt and mt.active then goto continue end
...
function M.pendingActiveSystem(world, battle)
    for id in pairs(world.entities) do
        ...
        if skill and skill.use then
            ...
            if cost > 0 and battle.resource then
                battle.resource = math.max(0, battle.resource - cost)
            end
        end
